...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<button id='btnEliminar' onclick=\"eliminarRegistro(" . $row["codigo_usuario"] . ")\">Eliminar</button>";
        echo "</td>";
        echo "</tr>";
...
<div id="formularioEdicion" style="display: none;">
        ...
        <form id="formularioEdicion">
            ...
            <input type="text" id="passuser"><br><br>
...
function editarRegistro(id) {
    ...
    function eliminarRegistro(id) {
        if (confirm("¿Estás seguro de que deseas eliminar este registro?")) {
            // Realiza una solicitud AJAX para eliminar el registro con el ID proporcionado
            var xhr = new XMLHttpRequest();
                xhr.open("GET", "eliminar_registro.php?id=" + id, true);
                xhr.onreadystatechange = function() {
                    if (xhr.readyState == 4 && xhr.status == 200) {
                        // Recargar la página para actualizar la tabla después de eliminar el registro
                        location.reload();
                    }
                };
                xhr.send();
